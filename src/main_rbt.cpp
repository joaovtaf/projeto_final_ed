...
BenchmarkResult executeSingleBenchmark(const std::string& directory, int numDocs) {
    ...
    for (int i = 0; i < numBuscas; ++i) {
        ...
        SearchResult searchResult = RBT::search(tree, processedWord);
...
void executeBenchmark(const std::string& directory) {
    ...
    //criar arquivo CSV
    std::ofstream csvFile("docs/benchmark_results_rbt.csv");
    if (!csvFile.is_open()) {
    ...
    std::cout << "Resultados salvos em: docs/benchmark_results_rbt.csv" << std::endl;
