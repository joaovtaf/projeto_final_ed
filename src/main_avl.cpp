...
//wrapper para a função insert da AVL
InsertResult insertWrapper(void* tree, const std::string& word, int documentId) {
    return AVL::insert(static_cast<BinaryTree*>(tree), word, documentId);
}
...
void executeSearch(BinaryTree* tree) {
    ...
    while (true) {
        ...
        SearchResult result = AVL::search(tree, processedWord);
...
BenchmarkResult executeSingleBenchmark(const std::string& directory, int numDocs) {
    ...
    for (int i = 0; i < numBuscas; ++i) {
        ...
        SearchResult searchResult = AVL::search(tree, processedWord);
...
void executeBenchmark(const std::string& directory) {
    ...
    //criar arquivo CSV
    std::ofstream csvFile("docs/benchmark_results_avl.csv");
    if (!csvFile.is_open()) {
    ...
    std::cout << "Resultados salvos em: docs/benchmark_results_avl.csv" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        std::cout << "Uso: ./avl <comando> <n_docs> <diretorio>" << std::endl;
        std::cout << std::endl;
        return 1;
    }
